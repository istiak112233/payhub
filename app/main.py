...
def startup() -> None:
    Path(s0.db_path).parent.mkdir(parents=True, exist_ok=True)
    db.init_db()
...
def dashboard(request: Request):
    ...
    if row["binance_api_key"] and row["binance_api_secret"]:
        ...
        bal_error = "Save your Binance API key and secret to see live balance."
    return templates.TemplateResponse(
        "dashboard.html",
        {
            "request": request,
...
def fail_payload(owner, inv, txid: str, raw: str) -> JSONResponse:
    msgs = db.get_messages(owner["id"])
    code = str(abs(hash(txid or "x")) % 9000 + 1000)
    tpl = msgs["fail_mismatch"] if "mismatch" in (raw or "").lower() else msgs["fail_not_found"]
    text = fill_msg(
        tpl,
        code=code,
        order_id=txid,
        amount=inv["amount"] if inv else "",
        currency=inv["currency"] if inv else "",
        pay_id=owner.get("binance_uid") or "",
        binance_name=owner.get("binance_name") or "",
        minutes=30,
        balance="",
    )
...
async def api_verify(body: dict, x_api_key: str | None = Header(default=None)):
    ...
    try:
        ...
        return JSONResponse({"ok": False, "error": raw, "message": text}, status_code=400)
    db.mark_paid(invoice_id, verified["transactionId"])
    payload = {
        "event": "PAYMENT_PAID",
        "invoice_id": invoice_id,
...
@app.post("/api/v1/invoice/{invoice_id}/cancel")
def api_cancel(invoice_id: str, x_api_key: str | None = Header(default=None)):
    bot, owner = _auth_bot(x_api_key)
    inv = db.get_invoice(invoice_id)
    if not inv or inv["user_id"] != owner["id"]:
        raise HTTPException(404, "invoice not found")
    ...
    with db.connect() as con:
        con.execute("UPDATE invoices SET status='CANCELLED' WHERE invoice_id=?", (invoice_id,))
